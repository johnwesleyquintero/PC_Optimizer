...
def apply_modern_theme(root: tk.Tk) -> ttk.Style:
    ...
    COLOR_CONTENT_BG = (
        "#FFFFFF"  # White background for content areas like Text, Treeview
    )
    ...
    style.configure(
        "TLabelframe", background=COLOR_BG, bordercolor=COLOR_BORDER, borderwidth=1
    )
    style.configure(
        "TLabelframe.Label",
        background=COLOR_BG,
        ...
        font=BOLD_FONT,
        padding=(5, 2),
    )  # Add some padding to labelframe titles
    ...
    style.map(
        "Sidebar.TButton",
        background=[
            ("active", COLOR_SIDEBAR_HOVER),
            ("!disabled", "hover", COLOR_SIDEBAR_HOVER),
        ],
        foreground=[("active", COLOR_TEXT), ("hover", COLOR_TEXT)],
    ...
    style.map(
        "TEntry",
        bordercolor=[("focus", COLOR_PRIMARY), ("hover", COLOR_BORDER_HOVER)],
    ...
    # --- Content Area Style ---
    style.configure(
        "Content.TFrame", background=COLOR_CONTENT_BG
    ...
    style.map(
        "TButton",
        background=[
            ("active", COLOR_PRIMARY_ACTIVE),  # Darker when pressed
    ...
    style.map(
        "Treeview.Heading",
        background=[
            ("active", COLOR_BORDER),  # Subtle feedback when clicking/hovering heading
            ("hover", COLOR_BORDER),
        ],
    )
    ...
    style.map(
        "Vertical.TScrollbar", background=[("active", COLOR_BORDER)]
    )  # When dragging
    ...
    style.map("Horizontal.TScrollbar", background=[("active", COLOR_BORDER)])
    ...
    style.map(
        ...
        background=[
            ("active", COLOR_PRIMARY_ACTIVE),  # Button part hover
    ...
    style.map(
        "TCheckbutton",
        indicatorcolor=[
            ("selected", COLOR_PRIMARY),  # Color when checked
            ("active", COLOR_PRIMARY),
        ],  # Color when hovered over
        background=[("active", COLOR_BG)],
    )  # Keep background consistent on hover
    ...
    style.map(
        "TRadiobutton",
        indicatorcolor=[("selected", COLOR_PRIMARY), ("active", COLOR_PRIMARY)],
        background=[("active", COLOR_BG)],
    )
