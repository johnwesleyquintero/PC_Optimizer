...
class GUIWorker:
    ...
    def __init__(self, max_queue_size: int = 100, name: str = "GUIWorkerThread"):
        ...
        self.task_queue: Queue[Tuple[Callable, Callable, tuple, dict]] = Queue(maxsize=max_queue_size)
    ...
    def start(self) -> None:
        ...
        with self._lock:
            if self._running:
                logger.warning(f"{self._thread_name}: Start called but already running.")
                return
            if self._worker_thread and self._worker_thread.is_alive():
                 logger.warning(f"{self._thread_name}: Start called but thread is still alive (unexpected state).")
                 # Optionally try to join previous thread here? Or raise error?
                 # For now, just log and proceed to create a new one.

            self._running = True
            self._stop_requested = False
            self._worker_thread = Thread(target=self._process_queue, name=self._thread_name, daemon=True)
    ...
    def stop(self, timeout: float = 2.0) -> None:
        ...
        should_join = False
        with self._lock:
            if not self._running and not (self._worker_thread and self._worker_thread.is_alive()):
                logger.info(f"{self._thread_name}: Stop called but already stopped.")
                return # Already stopped or never started

            if self._stop_requested:
                 logger.warning(f"{self._thread_name}: Stop called again while already stopping.")
                 # Allow join attempt again if needed
                 if self._worker_thread and self._worker_thread.is_alive():
                     should_join = True
            else:
                logger.info(f"{self._thread_name}: Stop requested. Signaling worker thread...")
        ...
        if should_join and self._worker_thread:
            ...
            logger.info(f"{self._thread_name}: Waiting up to {timeout}s for worker thread to join...")
            thread_to_join.join(timeout=timeout)
            if thread_to_join.is_alive():
                logger.warning(f"{self._thread_name}: Worker thread did not terminate within {timeout}s.")
    ...
    def add_task(self, task: Callable, callback: Callable[[Optional[Any], Optional[str]], None], *args, **kwargs) -> bool:
        ...
        with self._lock:
            if not self._running or self._stop_requested:
                ...
                msg = "Worker is not running." if not self._running else "Worker is stopping."
                logger.error(f"{self._thread_name}: Cannot add task '{getattr(task, '__name__', 'unknown')}': {msg}")
        ...
        try:
            ...
            logger.debug(f"{self._thread_name}: Added task '{getattr(task, '__name__', 'unknown')}' to queue.")
            return True
        except Full:
            logger.error(f"{self._thread_name}: Task queue is full. Cannot add task '{getattr(task, '__name__', 'unknown')}'.")
            # Optionally raise Full exception here instead of returning False
            # raise Full("Task queue is full")
            return False
        except Exception as e:
             logger.exception(f"{self._thread_name}: Unexpected error adding task '{getattr(task, '__name__', 'unknown')}': {e}")
             return False
    ...
    def _process_queue(self) -> None:
        ...
        while not self._stop_requested:
            try:
                ...
                task, callback, args, kwargs = self.task_queue.get(block=True, timeout=0.2)
            except Empty:
                # Timeout occurred, loop back to check self._stop_requested
                continue
            except Exception as e:
                 logger.exception(f"{self._thread_name}: Error getting task from queue: {e}")
            ...
            if self._stop_requested:
                logger.info(f"{self._thread_name}: Stop requested, discarding task '{getattr(task, '__name__', 'unknown')}'.")
            ...
            try:
                ...
                logger.debug(f"{self._thread_name}: Task '{task_name}' completed successfully.")
                self.result_queue.put((callback, result))
            except CancelledException as ce:
                 logger.warning(f"{self._thread_name}: Task '{task_name}' cancelled: {ce}")
                ...
                logger.exception(f"{self._thread_name}: Error executing task '{task_name}': {e}")
    ...
    def process_results(self, root: tk.Tk) -> None:
        ...
        if not root.winfo_exists():
            logger.warning(f"{self._thread_name}: Root window destroyed, stopping result processing.")
            ...
            while True:
                ...
                try:
                    if isinstance(result_or_error, Exception):
                        ...
                        error_msg = f"{type(result_or_error).__name__}: {str(result_or_error)}"
                        logger.warning(f"{self._thread_name}: Executing callback for failed task: {error_msg}")
                        callback(None, error=error_msg)
                    else:
                        # Task succeeded
                        logger.debug(f"{self._thread_name}: Executing callback for successful task.")
                        callback(result_or_error, error=None)
                except Exception as e:
                    # Catch errors *within* the callback function itself
                    logger.exception(f"{self._thread_name}: Error occurred within result callback '{getattr(callback, '__name__', 'unknown')}': {e}")
                finally:
        ...
        if self.is_running:
            root.after(100, lambda: self.process_results(root))
             ...
             logger.debug(f"{self._thread_name}: Worker not running, stopping result processing loop.")
    ...
    def _clear_queues(self) -> None:
        ...
        while not self.task_queue.empty():
            try:
                ...
                logger.debug(f"{self._thread_name}: Discarding task '{task_name}' from queue during cleanup.")
                self.task_queue.task_done()
            except Empty:
                break # Should not happen if not empty, but safety first
            except Exception as e:
                 logger.error(f"{self._thread_name}: Error clearing task queue item: {e}")
        ...
        while not self.result_queue.empty():
            try:
                ...
                break
                 ...
                 logger.error(f"{self._thread_name}: Error clearing result queue item: {e}")
                 break
