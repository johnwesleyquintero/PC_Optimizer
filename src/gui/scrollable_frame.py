...
class ScrollableFrame(ttk.Frame):
    ...
    def __init__(self, container: tk.Misc, v_scroll: bool = True, h_scroll: bool = False, *args: Any, **kwargs: Any):
    ...
    def _bind_mouse_wheel(self) -> None:
        ...
        if os_name == "Linux":
            ...
            self.canvas.bind("<Shift-Button-4>", self._on_mouse_wheel_horizontal, add='+')
            self.canvas.bind("<Shift-Button-5>", self._on_mouse_wheel_horizontal, add='+')
        elif os_name == "Windows":
            # Windows uses <MouseWheel>
            self.canvas.bind("<MouseWheel>", self._on_mouse_wheel, add='+')
            # Shift+Scroll for horizontal
            self.canvas.bind("<Shift-MouseWheel>", self._on_mouse_wheel_horizontal, add='+')
...
if __name__ == "__main__":
    ...
    scroll_frame_both = ScrollableFrame(root, v_scroll=True, h_scroll=True, relief=tk.GROOVE, borderwidth=1)
    ...
    for i in range(30):
        ...
        ttk.Label(content_frame, text=f"Label {i+1}", width=width, relief=tk.RIDGE).grid(
    ...
    long_text = "This is a very long piece of text designed to force horizontal scrolling if enabled. " * 5
    ttk.Label(content_frame, text=long_text, anchor='w').grid(row=30, column=0, columnspan=2, padx=5, pady=10, sticky='ew')
