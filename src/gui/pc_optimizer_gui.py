...
try:
    ...
    from ..core.performance_optimizer import PerformanceOptimizer # Assuming this exists
    ...
    class MockGUIWorker:
        def __init__(self): print("Mock GUIWorker initialized")
        def start(self): print("Mock GUIWorker started")
        def stop(self): print("Mock GUIWorker stopped")
        def add_task(self, task, callback, *args, **kwargs):
            ...
            try:
                ...
                print(f"Mock GUIWorker: Task {task.__name__} finished, calling callback")
                # Simulate calling back in main thread (not really async here)
                callback(result, None)
            except Exception as e:
                print(f"Mock GUIWorker: Task {task.__name__} failed: {e}")
                callback(None, str(e))
        def process_results(self, root): pass # No-op for mock
...
class AdaptiveGUI:
    def __init__(self):
        ...
        self.optimization_tasks = {
            "Clean Temporary Files": (self._clean_temp_files_task, self._on_task_complete),
            "Check Disk Usage": (self._check_disk_usage_task, self._on_task_complete),
            "List Startup Programs": (self._get_startup_programs_task, self._on_task_complete), # Renamed for clarity
            "Optimize Power Settings (Windows)": (self._optimize_power_settings_task, self._on_task_complete),
            "Run Disk Cleanup (Windows)": (self._run_disk_cleanup_task, self._on_task_complete),
        }
        self.create_widgets()
    ...
    def _apply_theme(self):
        ...
        style.configure('.', background=base_color, foreground=text_color, font=('Segoe UI', 9))
        style.configure('TLabel', background=base_color, foreground=text_color)
        style.configure('TFrame', background=base_color)
        style.configure('TLabelframe', background=base_color, foreground=text_color, bordercolor=primary_color)
        style.configure('TLabelframe.Label', background=base_color, foreground=text_color)
        ...
        style.map('TCheckbutton',
                  ...
                  background=[('active', adjust_color(base_color, 0.95))]) # Subtle hover on checkbutton background

        style.configure('TButton', background=primary_color, foreground='white', padding=(10, 5), borderwidth=0)
        style.map('TButton',
                  background=[('active', hover_color), ('disabled', adjust_color(primary_color, 1.2))])

        style.configure('Log.TText', background='white', foreground=text_color, font=('Consolas', 9)) # Specific style for log
    ...
    def create_widgets(self):
        ...
        for option_name in self.optimization_tasks.keys():
            ...
            cb = ttk.Checkbutton(options_frame, text=option_name, variable=self.option_vars[option_name])
        ...
        self.log_text = scrolledtext.ScrolledText(log_frame, wrap=tk.WORD, height=15, width=70, relief=tk.FLAT)
        self.log_text.grid(row=0, column=0, sticky=(tk.N, tk.S, tk.E, tk.W))
        self.log_text.configure(font=('Consolas', 9), state=tk.DISABLED) # Start disabled
        ...
        self.log_text.tag_configure('task_start', foreground='blue', font=('Consolas', 9, 'bold'))

        # --- Button Frame ---
        button_frame = ttk.Frame(main_frame, padding=(0, 10, 0, 0)) # Padding top only
        button_frame.grid(row=2, column=0, sticky=(tk.E, tk.W))

        self.run_button = ttk.Button(button_frame, text="Run Selected Optimizations", command=self.run_optimizations)
    ...
    def run_optimizations(self):
        ...
        for option_name in selected_options:
            if option_name in self.optimization_tasks:
                ...
                task_callback = lambda result, error, name=option_name: callback_func(name, result, error)
                ...
                if added:
                    ...
                    self._log(f"Failed to queue task: {option_name}. Worker queue might be full.", 'error')
            else:
    ...
    def _clean_temp_files_task(self) -> dict:
        ...
        try:
            ...
            message = "Temporary files cleaned successfully." if success else "Failed to clean temporary files (or none found)."
    ...
    def _get_startup_programs_task(self) -> str:
        ...
        if platform.system() != "Windows":
            logger.warning("Worker: get_startup_programs task skipped (Not on Windows).")
    ...
    def _optimize_power_settings_task(self) -> dict:
        ...
        if platform.system() != "Windows":
            ...
            return {'success': False, 'message': "Power settings optimization is only available on Windows."}
    ...
    def on_close(self):
        ...
        try:
            if self.run_button.winfo_exists(): self.run_button.configure(state=tk.DISABLED)
            if self.exit_button.winfo_exists(): self.exit_button.configure(state=tk.DISABLED)
        except tk.TclError:
