...
from .config_manager import ConfigManager # Using ConfigManager for consistency with SentinelCore
...
class PerformanceOptimizer(BasePerformanceOptimizer):
    ...
    DEFAULT_TASKS_CONFIG = {
        "memory_optimization": {
            "function": "adjust_memory_usage", "priority": 1, "critical": True, "timeout": 300, "enabled": True
        },
        "temp_cleanup": {
            "function": "clean_temp_files", "priority": 2, "critical": False, "timeout": 600, "enabled": True
        },
        "disk_defrag": { # Example: Added disk defrag task (Windows only)
            "function": "defragment_disk", "priority": 3, "critical": False, "timeout": 3600, "enabled": True, "os": "Windows"
        },
        "windows_theme_perf": { # Example: Windows theme adjustment task
             "function": "adjust_windows_theme_performance", "priority": 4, "critical": False, "timeout": 60, "enabled": True, "os": "Windows", "params": {"optimize_for_performance": True}
        }
    }
    ...
    def __init__(self, config_manager: Optional[ConfigManager] = None):
        ...
        self._theme_settings: Dict[str, Any] = {} # Store original theme settings
    ...
    def initialize(self, config: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            if config:
                 self.logger.warning("Initializing with direct config dict - prefer using ConfigManager update methods.")
            ...
            self.logger.error(f"Failed to initialize performance optimizer: {e}", exc_info=True)
            self._status = "error"
            return False
    ...
    def _init_windows_theme_performance(self) -> None:
        ...
        if not _HAS_WINREG:
            self.logger.warning("Winreg module not found, cannot initialize Windows theme settings.")
            return
        try:
            theme_keys = {
                "SystemUsesLightTheme": (winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Windows\CurrentVersion\Themes\Personalize"),
                "AppsUseLightTheme": (winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Windows\CurrentVersion\Themes\Personalize"),
                "EnableTransparency": (winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Windows\CurrentVersion\Themes\Personalize"),
                # Add other relevant keys like visual effects if needed
                "VisualFXSetting": (winreg.HKEY_CURRENT_USER, r"Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects"),
            }
            ...
            for setting, (hkey, path) in theme_keys.items():
                try:
                    ...
                    self.logger.debug(f"Registry key or value not found for {setting} at {path}. Skipping.")
                except OSError as e:
                    self.logger.warning(f"Could not read {setting} from registry path {path}: {e}")

            self.logger.info("Windows theme performance settings initialized.")
        except Exception as e:
            self.logger.error(f"Failed to initialize Windows theme performance settings: {e}", exc_info=True)
            # Don't raise here, initialization should still succeed if possible

    def adjust_windows_theme_performance(self, optimize_for_performance: bool = True) -> Dict[str, Any]:
        """
        ...
        if platform.system() != "Windows":
            ...
            return result
        if not _HAS_WINREG:
            result["details"] = "Winreg module not found, cannot adjust Windows theme settings."
        ...
        try:
            ...
            visual_fx_key_path = r"Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects"

            for setting, value in target_settings.items():
                key_path = theme_key_path if setting != "VisualFXSetting" else visual_fx_key_path
                try:
                    ...
                    result["success"] = False # Mark as failed but continue trying others
            ...
            result["success"] = not any(err in result["details"] for err in ["Failed", "not found"])
    ...
    def optimize_system(self, profile: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"Starting system optimization with profile: {profile or 'default'}")
            self._validate_optimization_ready()
            tasks_config = self._get_tasks_for_profile(profile)

            if not tasks_config:
                self.logger.warning("No optimization tasks found or enabled for this profile.")
            ...
            self.logger.info(f"Optimization finished. Success: {report['success']}. Duration: {report['duration_seconds']}s")
            return report
            ...
            self._status = "failed"
            self._last_run_result = {"success": False, "error": str(opt_err), "details": opt_err.details}
    ...
    def _validate_optimization_ready(self) -> None:
        ...
        try:
            if not self._check_system_resources():
                 self.logger.warning("System resources are currently high, optimization might be less effective.")
             ...
             self.logger.warning(f"Failed to check system resources: {e}. Proceeding with optimization.")
    ...
    def _check_system_resources(self) -> bool:
        ...
        try:
            cpu_threshold = float(self.config.get("Performance", "readiness_cpu_threshold", fallback=95.0))
            mem_threshold = float(self.config.get("Performance", "readiness_mem_threshold", fallback=98.0))
            min_mem_mb = int(self.config.get("Performance", "readiness_min_mem_mb", fallback=256))

            cpu_percent = psutil.cpu_percent(interval=0.5) # Shorter interval for readiness check
            ...
            if not sufficient:
                self.logger.warning(f"Resource check: CPU={cpu_percent:.1f}% (Threshold={cpu_threshold}%), "
                                    f"Mem={memory.percent:.1f}% (Threshold={mem_threshold}%), "
                                    f"AvailMem={memory.available / (1024*1024):.0f}MB (Min={min_mem_mb}MB)")
            return sufficient
        except Exception as e:
            self.logger.warning(f"Failed to check system resources during readiness check: {e}")
    ...
    def _get_thread_count(self) -> int:
        ...
        try:
            ...
            config_max_threads = self.config.getint("Performance", "max_threads", fallback=max(1, cpu_count - 1))
            ...
            self.logger.debug(f"Determined thread count: {thread_count} (ConfigMax: {config_max_threads}, CPUCores: {cpu_count})")
            return thread_count
        except Exception as e:
            self.logger.error(f"Failed to determine thread count: {e}. Defaulting to 1.", exc_info=True)
            return 1

    def _get_tasks_for_profile(self, profile: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        """
        profile_name = profile or self.config.get("Profiles", "default", fallback="default")
        ...
        if self.config.has_section(profile_section):
            ...
            for task_name, default_config in self.DEFAULT_TASKS_CONFIG.items():
                ...
                profile_setting = defined_tasks.get(task_name.lower()) # Config keys are lowercased
                if profile_setting is not None:
                    try:
                        ...
                        if len(parts) > 1: task_config["priority"] = int(parts[1])
                        if len(parts) > 2: task_config["timeout"] = int(parts[2])
                        if len(parts) > 3: # Parse simple key=value params
                            ...
                            for kv in parts[3:]:
                                if '=' in kv:
                                    ...
                                    if v.lower() == 'true': params[k.strip()] = True
                                    elif v.lower() == 'false': params[k.strip()] = False
                                    elif v.isdigit(): params[k.strip()] = int(v)
                                    elif v.replace('.', '', 1).isdigit(): params[k.strip()] = float(v)
                                    else: params[k.strip()] = v
                            task_config["params"] = params

                    except (ValueError, IndexError) as e:
                         self.logger.warning(f"Invalid format for task '{task_name}' in profile '{profile_name}': '{profile_setting}'. Using defaults. Error: {e}")
                         # Keep default enabled/priority/timeout if parsing fails
                         task_config["enabled"] = default_config.get("enabled", False) # Revert to default enabled state
                else:
                ...
                os_match = "os" not in task_config or task_config["os"] == platform.system()
                if task_config.get("enabled", False) and os_match:
                    task_config["name"] = task_name
                    if task_config["function"] not in self._tasks:
                        self.logger.error(f"Task function '{task_config['function']}' for task '{task_name}' is not implemented.")
                        continue # Skip unimplemented tasks
                    tasks_to_run.append(task_config)
                    self.logger.debug(f"Added task '{task_name}' from profile '{profile_name}' with config: {task_config}")

        else:
            self.logger.warning(f"Profile section '{profile_section}' not found. Using default tasks.")
            ...
            for task_name, default_config in self.DEFAULT_TASKS_CONFIG.items():
                 ...
                 if task_config.get("enabled", False) and os_match:
                    ...
                    if task_config["function"] not in self._tasks:
                        ...
                        continue
                    tasks_to_run.append(task_config)
                    self.logger.debug(f"Added default task '{task_name}' with config: {task_config}")

        # Validate and sort tasks
        valid_tasks = []
        for task in tasks_to_run:
            if not all(k in task for k in ("name", "function", "priority")):
                self.logger.error(f"Invalid task configuration structure for {task.get('name', 'unknown')}: Missing required keys. Skipping.")
                continue
    ...
    def _execute_tasks(
        ...
        for task_config in tasks:
            ...
            self.logger.debug(f"Submitting task: {task_name} with params: {task_params}, timeout: {task_timeout}s")
            future = executor.submit(self._optimize_task_wrapper, task_name, task_func, task_params)
        ...
        for future in concurrent.futures.as_completed(futures.keys()):
            ...
            try:
                ...
                self.logger.info(f"Task '{task_name}' completed. Success: {result.get('success')}")
            except FuturesTimeoutError:
                error_msg = f"Task '{task_name}' timed out after {task_timeout} seconds."
                self.logger.error(error_msg)
                results.append({"name": task_name, "success": False, "error": "timeout", "details": error_msg, "critical": task_config.get("critical", False)})
                ...
                self.logger.error(error_msg, exc_info=True)
                results.append({"name": task_name, "success": False, "error": str(e), "details": error_msg, "critical": task_config.get("critical", False)})

        return results

    def _optimize_task_wrapper(self, task_name: str, task_func: Callable, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if isinstance(task_result, dict) and "success" in task_result:
                ...
                result_dict = {"success": task_result, "details": "Task returned boolean."}
            else: # Handle unexpected return types
                 self.logger.warning(f"Task '{task_name}' returned unexpected type: {type(task_result)}. Assuming success=True.")
                 result_dict = {"success": True, "details": f"Task returned non-standard type: {type(task_result)}"}

            result_dict["name"] = task_name
            result_dict["duration_seconds"] = round(time.monotonic() - start_time, 2)
            self.logger.info(f"Finished task: {task_name} in {result_dict['duration_seconds']:.2f}s. Success: {result_dict['success']}")
            return result_dict

        except (OptimizationError, ConfigurationError, TaskExecutionError, MemoryOptimizationError,
                  FileCleanupError, DiskOperationError, StartupManagementError) as opt_err:
            # Catch specific optimization errors raised by tasks
            duration = round(time.monotonic() - start_time, 2)
            error_msg = f"Task '{task_name}' failed after {duration:.2f}s: {opt_err}"
            self.logger.error(error_msg, exc_info=True)
            return {"name": task_name, "success": False, "error": type(opt_err).__name__, "details": str(opt_err), "duration_seconds": duration}
        except Exception as e:
            ...
            self.logger.error(error_msg, exc_info=True)
            return {"name": task_name, "success": False, "error": "unexpected", "details": error_msg, "duration_seconds": duration}
        finally:
    ...
    def _generate_optimization_report(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            if result.get("success", False):
                ...
                failure_detail = {
                    ...
                    "critical": result.get("critical", False), # Check if the task was marked critical
        ...
        if not report["success"]:
             report["message"] = f"Optimization completed with {report['tasks_failed']} failed task(s)."
    ...
    def adjust_memory_usage(self) -> Dict[str, Any]:
        ...
        result = {"success": False, "details": "", "state": "unknown", "actions_taken": []}
        try:
            ...
            if self.config.has_section(config_section):
                for key, default_val in self.DEFAULT_MEMORY_CONFIG.items():
                    if isinstance(default_val, bool):
                        mem_cfg[key] = self.config.getboolean(config_section, key, fallback=default_val)
                    elif isinstance(default_val, int):
                        mem_cfg[key] = self.config.getint(config_section, key, fallback=default_val)
                    elif isinstance(default_val, float):
                         mem_cfg[key] = self.config.getfloat(config_section, key, fallback=default_val)
                    else: # String priorities
                         mem_cfg[key] = self.config.get(config_section, key, fallback=default_val)
            else:
                 self.logger.warning(f"Configuration section '[{config_section}]' not found. Using default memory settings.")
            ...
            self.logger.info(f"Memory state: {current_state} (Available: {available_gb:.2f}GB, Usage: {usage_percent:.1f}%)")
            ...
            current_max_threads = self.config.getint("Performance", "max_threads", fallback=multiprocessing.cpu_count())
            if current_max_threads != state_max_threads:
                try:
                    self.config.set("Performance", "max_threads", str(state_max_threads))
                    # No need to call save_config here, let cleanup handle it or rely on ConfigManager's auto-save
                    self.logger.info(f"Set max_threads to {state_max_threads} due to memory state '{current_state}'.")
                    result["actions_taken"].append(f"set_max_threads={state_max_threads}")
                except Exception as e:
            ...
            if platform.system() == "Windows":
                ...
                target_priority = priority_map.get(state_priority_str.lower(), psutil.NORMAL_PRIORITY_CLASS)
                try:
                    current_process = psutil.Process()
                    current_priority = current_process.nice()
                    if current_priority != target_priority:
                        current_process.nice(target_priority)
                        self.logger.info(f"Set process priority to '{state_priority_str}' ({target_priority}) due to memory state '{current_state}'.")
                        result["actions_taken"].append(f"set_priority={state_priority_str}")
            ...
            if state_clear_cache:
                self.logger.info(f"Triggering system cache clear due to memory state '{current_state}'.")
                cache_clear_result = self._clear_system_cache()
                result["actions_taken"].append(f"clear_cache={cache_clear_result['success']}")
                if not cache_clear_result["success"]:
                     result["warning"] = result.get("warning", "") + f" Cache clearing failed: {cache_clear_result['details']}"


            result["success"] = True # Mark success if actions were attempted
            result["details"] = f"Memory adjustments applied for state '{current_state}'. Available: {available_gb:.2f}GB."
            return result

        except configparser.Error as e:
             error_msg = f"Invalid memory configuration: {e}"
             self.logger.error(error_msg, exc_info=True)
             raise ConfigurationError(error_msg, {"section": "MemoryOptimization"}) from e
        except Exception as e:
            error_msg = f"Failed to adjust memory usage: {e}"
            self.logger.error(error_msg, exc_info=True)
            # Raise a specific error if adjustment fails critically
            raise MemoryOptimizationError(error_msg, {"available_gb": available_gb, "usage_percent": usage_percent}) from e
        ...
        try:
            if platform.system() == "Windows":
                ...
                try:
                    ...
                    self.logger.warning(f"Failed to flush DNS cache (requires admin?): {dns_err}")
                ...
                self.logger.info("Windows Update cache clearing is complex and requires admin rights, skipping for now.")
                result["details"] += "WU cache clear skipped. "


                # Add other cache clearing methods if known (e.g., specific app caches)

                result["success"] = True # Mark success if attempts were made, even if some failed
                 ...
                 if os.geteuid() == 0:
                     try:
                         ...
                         self.logger.info("Linux caches dropped successfully (pagecache, dentries, inodes).")
                         result["details"] = "Linux caches dropped."
                         result["success"] = True
                     except IOError as e:
                         self.logger.warning(f"Failed to drop Linux caches (permission error?): {e}")
                         result["details"] = f"Failed to drop Linux caches: {e}"
                 else:
                     self.logger.warning("Cannot drop Linux caches: requires root privileges.")
                 ...
                 try:
                     import subprocess
                     subprocess.run(['sudo', 'purge'], check=True, capture_output=True, text=True)
                     ...
                     self.logger.warning(f"Failed to purge macOS memory (requires sudo?): {purge_err}")
                     result["details"] = f"Failed to purge macOS memory: {purge_err}"

            else:
                self.logger.info(f"Cache clearing not implemented for OS: {platform.system()}")
    ...
    def clean_temp_files(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.has_section(config_section):
                 cleanup_cfg["critical_disk_usage_percent"] = self.config.getfloat(config_section, "critical_disk_usage_percent", fallback=cleanup_cfg["critical_disk_usage_percent"])
                 cleanup_cfg["high_disk_usage_percent"] = self.config.getfloat(config_section, "high_disk_usage_percent", fallback=cleanup_cfg["high_disk_usage_percent"])
                 cleanup_cfg["critical_age_threshold_days"] = self.config.getint(config_section, "critical_age_threshold_days", fallback=cleanup_cfg["critical_age_threshold_days"])
                 cleanup_cfg["high_age_threshold_days"] = self.config.getint(config_section, "high_age_threshold_days", fallback=cleanup_cfg["high_age_threshold_days"])
                 cleanup_cfg["normal_age_threshold_days"] = self.config.getint(config_section, "normal_age_threshold_days", fallback=cleanup_cfg["normal_age_threshold_days"])
                 # Load patterns (assuming JSON or comma-separated in config)
                 try:
                     patterns_str = self.config.get(config_section, "patterns", fallback=str(cleanup_cfg["patterns"]))
                     # Basic parsing assuming dict-like string or JSON string
                     import json
                     try:
                         cleanup_cfg["patterns"] = json.loads(patterns_str.replace("'", '"'))
                     except json.JSONDecodeError:
                          self.logger.warning(f"Could not parse patterns from config string: {patterns_str}. Using defaults.")
                          cleanup_cfg["patterns"] = self.DEFAULT_CLEANUP_CONFIG["patterns"]
                 except Exception as pattern_err:
                     self.logger.warning(f"Error loading patterns from config: {pattern_err}. Using defaults.")
                     cleanup_cfg["patterns"] = self.DEFAULT_CLEANUP_CONFIG["patterns"]

                 # Load skip prefixes
                 skip_prefixes_str = self.config.get(config_section, "skip_prefixes", fallback=",".join(cleanup_cfg["skip_prefixes"]))
                 cleanup_cfg["skip_prefixes"] = [p.strip() for p in skip_prefixes_str.split(',') if p.strip()]
            else:
                 self.logger.warning(f"Configuration section '[{config_section}]' not found. Using default cleanup settings.")


            # Identify temporary directories
            env_vars = ["TEMP", "TMP"]
            if platform.system() == "Windows":
                env_vars.extend(["LOCALAPPDATA", "USERPROFILE"]) # Look in AppData\Local\Temp too
            elif platform.system() == "Linux":
            ...
            for var in env_vars:
                if path_str := os.environ.get(var):
                    path = Path(path_str)
                    if var in ["LOCALAPPDATA", "USERPROFILE"] and platform.system() == "Windows":
            ...
            self.logger.info(f"Identified temp directories to scan: {[str(d) for d in temp_dirs_to_clean]}")
            ...
            try:
                ...
                total_space_before = usage.used # Rough estimate
                 ...
                 self.logger.warning(f"Could not get disk usage for {temp_dirs_to_clean[0]}: {e}. Using normal age threshold.")
            ...
            self.logger.info(f"Disk usage at {used_percent:.1f}%. Applying '{cleanup_level}' cleanup (Age > {age_threshold_days} days).")
            result["details"] = f"Cleanup level: {cleanup_level} (Age > {age_threshold_days} days)."
                ...
                try:
                    for item in os.scandir(temp_dir):
                        try:
                            ...
                            # Check skip prefixes
                            if any(item.name.lower().startswith(p) for p in cleanup_cfg["skip_prefixes"]):
                                self.logger.debug(f"Skipping '{item.name}' due to skip prefix.")
                            ...
                            if item.is_file() or item.is_symlink():
                                ...
                                dir_size = sum(f.stat().st_size for f in item_path.glob('**/*') if f.is_file()) # Approx size
                                shutil.rmtree(item_path, ignore_errors=True)
                                result["dirs_removed"] += 1
                                result["space_freed_bytes"] += dir_size # Add estimated dir size
                                self.logger.debug(f"Removed directory: {item_path}")

                        except PermissionError as pe:
                            result["errors"].append({"path": item.path, "error": f"Permission denied: {pe}"})
                            self.logger.debug(f"Permission denied for {item.path}: {pe}")
                        except OSError as oe:
                            result["errors"].append({"path": item.path, "error": f"OS error: {oe}"})
                            self.logger.warning(f"Failed to process {item.path}: {oe}")
                        except Exception as item_err:
                             result["errors"].append({"path": item.path, "error": f"Unexpected error: {item_err}"})
                             self.logger.warning(f"Unexpected error processing {item.path}: {item_err}", exc_info=True)

                except PermissionError as dir_pe:
                     result["errors"].append({"path": str(temp_dir), "error": f"Permission denied accessing dir: {dir_pe}"})
                     self.logger.warning(f"Permission denied accessing directory {temp_dir}: {dir_pe}")
            ...
            result["details"] += (f" Removed {result['files_removed']} files, {result['dirs_removed']} dirs. "
    ...
    def defragment_disk(self, drive_letter: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Starting defragmentation analysis for drive {target_drive}:")
            ...
            try:
                ...
                analysis_output = subprocess.run(analyze_cmd, check=True, capture_output=True, text=True, timeout=300, creationflags=subprocess.CREATE_NO_WINDOW)
                self.logger.info(f"Defrag analysis output for {target_drive}:\n{analysis_output.stdout}")
                result["details"] = f"Analysis complete for {target_drive}:. Output logged."
                ...
                defrag_cmd = ['defrag', f'{target_drive}:', '/U', '/V'] # Add /O for optimization if needed
                self.logger.info(f"Starting defragmentation for drive {target_drive}: (requires admin)")
                self.logger.debug(f"Running command: {' '.join(defrag_cmd)}")
                defrag_output = subprocess.run(defrag_cmd, check=True, capture_output=True, text=True, timeout=3600, creationflags=subprocess.CREATE_NO_WINDOW) # Longer timeout
                self.logger.info(f"Defrag execution output for {target_drive}:\n{defrag_output.stdout}")
                result["details"] = f"Defragmentation command executed for {target_drive}:. Output logged."
                result["success"] = True # Assume success if command runs without error
                 ...
                 self.logger.error(error_msg)
                 result["details"] = error_msg
                 raise DiskOperationError(error_msg, {"drive": target_drive, "return_code": cpe.returncode}) from cpe
            except FileNotFoundError:
    ...
    def get_disk_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for partition in partitions:
                ...
                if 'cdrom' in partition.opts or 'ro' in partition.opts or not partition.fstype:
                    self.logger.debug(f"Skipping partition: {partition.device} (Type: {partition.fstype}, Opts: {partition.opts})")
                    continue
                ...
                try:
                    ...
                    if usage_percent >= 95:
                        status = "critical"
                        warnings.append(f"Critical disk usage ({usage_percent:.1f}%) on {partition.mountpoint} ({partition.device})")
                    elif usage_percent >= 90:
                        status = "warning"
                        warnings.append(f"High disk usage ({usage_percent:.1f}%) on {partition.mountpoint} ({partition.device})")
                    ...
                    self.logger.warning(f"Permission denied accessing disk usage for {partition.mountpoint}")
                    inaccessible_partitions.append({"device": partition.device, "mountpoint": partition.mountpoint, "reason": "permission_denied"})
                except FileNotFoundError:
                     self.logger.warning(f"Mount point not found for {partition.device}: {partition.mountpoint} (possibly disconnected?)")
                     inaccessible_partitions.append({"device": partition.device, "mountpoint": partition.mountpoint, "reason": "mount_point_not_found"})
                except OSError as e:
                    # Catch specific OS errors like "[Errno 19] No such device"
                    self.logger.warning(f"OS error getting disk usage for {partition.mountpoint}: {e}")
                    inaccessible_partitions.append({"device": partition.device, "mountpoint": partition.mountpoint, "reason": f"os_error_{e.errno}"})
                except Exception as e:
                     # Catch any other unexpected errors for a specific partition
                     self.logger.error(f"Unexpected error getting usage for {partition.mountpoint}: {e}", exc_info=True)
                     inaccessible_partitions.append({"device": partition.device, "mountpoint": partition.mountpoint, "reason": f"unexpected_{type(e).__name__}"})


            if not success and not disk_info:
                # Only raise if we couldn't read *any* partition info
                error_details = {"inaccessible_partitions": inaccessible_partitions, "os_type": platform.system()}
                raise DiskOperationError("No accessible disk partitions found or read successfully.", error_details)
    ...
    def manage_startup_programs(self, action: str, program_name: str, program_path: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if action == "enable" and not Path(program_path).is_file():
             raise ValueError(f"Program path does not exist or is not a file: {program_path}")


        try:
            if platform.system() == "Windows":
                if not _HAS_WINREG:
                     raise StartupManagementError("Winreg module not found, cannot manage Windows startup items.")
                op_result = self._manage_windows_startup(action, program_name, program_path)
                result.update(op_result) # Merge results
                 ...
                 raise StartupManagementError("macOS startup management via launchd not implemented yet.")
            elif platform.system() == "Linux":
                 # Requires creating/deleting .desktop files in ~/.config/autostart
                 raise StartupManagementError("Linux startup management via autostart not implemented yet.")
            else:
                raise StartupManagementError(f"Startup management not supported on this OS: {platform.system()}")

            if result["success"]:
                 self.logger.info(f"Successfully {action}d startup program '{program_name}'. Details: {result['details']}")
            else:
                 self.logger.error(f"Failed to {action} startup program '{program_name}'. Error: {result['error']}")
            ...
            self.logger.error(error_msg, exc_info=True)
            raise StartupManagementError(error_msg, {"action": action, "program": program_name}) from e

    def _manage_windows_startup(self, action: str, program_name: str, program_path: Optional[str]) -> Dict[str, Any]:
            ...
            with winreg.OpenKey(hkey, key_path, 0, access_mask) as key:
                if action == "disable":
                    try:
                        ...
                        result["success"] = True
                        result["details"] = f"Disabled startup program '{program_name}'."
                    except FileNotFoundError:
                        result["success"] = True # Considered success if already disabled
                        result["details"] = f"Startup program '{program_name}' was already disabled or not found."
                    except OSError as e:
                         raise StartupManagementError(f"Failed to delete registry value '{program_name}': {e}", {"winerror": e.winerror}) from e
                    ...
                    try:
                        ...
                        result["success"] = True
                        result["details"] = f"Enabled startup program '{program_name}' with path '{program_path}'."
                    except OSError as e:
                         raise StartupManagementError(f"Failed to set registry value '{program_name}': {e}", {"winerror": e.winerror}) from e
        except FileNotFoundError:
             ...
             error_msg = f"Unexpected registry error managing startup item '{program_name}': {e}"
             self.logger.error(error_msg, exc_info=True)
    ...
    def _cleanup_executor(self) -> bool:
        ...
        if self._executor:
            ...
            try:
                self._executor.shutdown(wait=True, cancel_futures=True) # Attempt to cancel pending
                self._executor = None
                self.logger.debug("Thread pool executor shut down.")
                return True
            except Exception as e:
                self.logger.error(f"Failed to properly shutdown executor: {e}", exc_info=True)
                return False
    ...
    def cleanup(self) -> bool:
        ...
        if platform.system() == "Windows" and self._theme_settings:
             self.logger.info("Attempting to restore original Windows theme settings during cleanup.")
             restore_result = self.adjust_windows_theme_performance(optimize_for_performance=False)
             if not restore_result["success"]:
                  self.logger.warning(f"Failed to restore Windows theme settings during cleanup: {restore_result['details']}")
        ...
        if all_ok:
            self.logger.info("Performance optimizer cleanup completed successfully.")
             ...
             self.logger.warning("Performance optimizer cleanup completed with some issues.")
